...
class MenuIcon extends React.Component<IIconName, {}> {
  public render(): React.ReactElement<{}> {
    ...
    //Filter menu items and remove non required infrastructure stages
    const _exclusionList: any[] = this.props.ExcludedMenuItems ? this.props.ExcludedMenuItems : [];    
    let _menuItemArray = [];
    if (_exclusionList.length > 0){
      stages.forEach(stage => {
        if(stage.subtile){
          if(_exclusionList.some(exElement => (exElement === stage.subtile))){
            stage.hidden = true;            
          }
        }        
        _menuItemArray.push(stage);        
      });      
    }
    else{
      _menuItemArray = stages;
    }
      

    return (
      <div className={appMenu}>
        {_menuItemArray.map((menu) => {
